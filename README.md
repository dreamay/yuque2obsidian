...
- 自动**下载图片/附件**到各知识库的 `assets/`，并把 Markdown 链接改成本地相对路径
- **语雀内部文档链接**自动替换为 Obsidian `[[...]]` 内部链接
- **Lake 格式**（画板、思维导图、表格）转换为 Markdown 兼容格式
...
4. **Lake 格式**：画板、思维导图已转换为占位提示（含原文链接），表格尝试转为 Markdown 表格；若 `body` 为空会回退到 `body_html` 的基础 HTML→Markdown 转换。
5. **语雀内部链接**：已自动将 `https://www.yuque.com/.../docs/xxx` 替换为 Obsidian `[[...]]` 内部链接。同知识库链接通过目录树即时解析，跨知识库链接通过 SQLite 状态表解析；若目标文档尚未导出，则保留原始 URL。
6. **API 覆盖范围**：本项目基于语雀官方 OpenAPI v2 实现核心导出链路。与社区常见的非官方 Web API 方案（如 yuque-dl、yuque-tools）相比，更稳定可持续；未覆盖的评论导出、小记导出、密码验证等功能，可后续作为扩展模块接入。
